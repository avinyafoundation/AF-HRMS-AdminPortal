...
class Applicant {
    ...
    {
            if(id != null) 'id': id,
           if(positions_vacant_id != null) 'positions_vacant_id': positions_vacant_id,
...
Future<List<Applicant>> fetchApplicants() async {
    final response =
        await http.get(Uri.parse(AppConfig.hrmApiUrl + '/sms/hrm/applicants'),
...
Future<Applicant> fetchApplicant(String id) async {
    final response =
        await http.get(Uri.parse(AppConfig.hrmApiUrl + '/sms/hrm/applicants/$id'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'accept': 'application/json',
          'API-Key': AppConfig.hrmApiKey,
    ...
    if (response.statusCode == 200) {
        var resultsJson = json.decode(response.body).cast<Map<String, dynamic>>();
        Applicant applicant = await resultsJson
            .map<Applicant>((json) => Applicant.fromJson(json));
        return applicant;
    } else {
        throw Exception('Failed to load Applicant');
